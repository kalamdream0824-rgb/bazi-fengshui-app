...
3. ~~**E2E 冒烟测试**~~ **已交付（2026-08-16）**——`e2e/run_e2e.py`（Playwright：注册登录→排盘→会员购买→分享→历史→刷新→退出登录清空→设置-关于合规，**8/8 通过**）；顺带修复刷新兜底 bug（v0.31）。
...
8. 小项：~~**关于与合规声明页**~~ **已交付（2026-08-16，v0.36）**——设置页四块内容 + 版本号，首页/我的齿轮入口接入；剩余「设置-流派」、「产品图'时辰边界提示'待办口径清理」。
9. ~~**新人解读内容深度扩展**（P0-P2）~~ **已全部交付（2026-08-15~16）**——词库+规则两层架构（`lib/explainer`），P0 旺衰/自坐/副星/大运十神 → P1 十神组合/神煞落宫 → P2 胎元命宫身宫/流年冲合 → **P3 总览一句话档案（buildProfile）**，每盘组合型解读 ≥3 条起步、P2 后 ≥7 条；详见 frontend-design v0.27-v0.30。
...
| commit | 内容 |
|---|---|
| 5de5136 | 设置页关于与合规 v0.36（/settings 路由 + 齿轮入口 + 合规四块 + E2E 8/8） |
| 756e984 | 首页黄历回退原版（浅签实测不佳，其余 v0.35 视觉保留） |
| 6899424 | 墨韵纸底视觉落地 v0.35（浅签首页/日柱高亮/大运时间轴/暖墨导航/笺纸解读） |
| 8a7c274 | 退出登录清空本地历史与当前命盘（隐私修复）+ E2E 登出场景 |
