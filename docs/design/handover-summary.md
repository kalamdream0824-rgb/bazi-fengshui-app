...
- **后端**：排盘 API + fixtures 一致性、真太阳时后端权威计算、账号 JWT/BCrypt/登录限流、记录 CRUD/隔离/去重、会员权益/兑换码/订单/模拟支付（`/orders`、`/pay/mock-success/{orderId}`、`/pay/callback` 占位）、P0 安全加固、P1 MySQL + 请求日志；神煞/合婚/每日运势仍由前端实现。
...
11. **解读内容专业性质保**（P0，盈利质量根基）：现有词库为「通识意象」级别、无经典出处，命书/解读的付费价值与可信度不足。方向：① 词库出处化——按《渊海子平》《三命通会》《滴天髓》《穷通宝鉴》通行论述重写高频词条（十神/天干/旺衰/大运），每条带出处字段；② 明确「子平格局派」口径声明；③ 质量测试升级——禁空泛套话占比、每盘必须引用具体干支/十神/神煞组合、出处字段非空；④ 内容分级——入门版（通识意象，免费）与专业版（经典出处断语，命书/付费）；⑤ **专家审校**（需外部资源）：整理待审校清单（词条/规则/争议点）供命理从业者逐条批注，AI 无法替代。

## 6. 最近提交（新 → 旧）

| commit | 内容 |
|---|---|
| 74c334c | 命书 PDF 升级五章完整版 v0.37（复用组合型解读，实测 6 页 A4） |
