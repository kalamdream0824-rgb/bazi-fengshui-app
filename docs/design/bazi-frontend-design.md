...
| 项目 | 内容 |
|---|---|
| 文档版本 | v0.4（已实现：M0.5 + M0.8 + 表单组件 + 真太阳时；进行中：A2 神煞、A1 命书 PDF） |
| 日期 | 2026-08-08 |
| 适用范围 | 前端（apps/web）开发 |
...
- **A2 神煞（先做）**：前端内置约 20 个高频神煞（桃花/驿马/华盖/羊刃/禄神/天乙贵人/文昌/劫煞/亡神/将星/空亡/孤辰寡宿/红艳/流霞/金舆/魁罡/十恶大败/阴阳差错/孤鸾煞等），以年支/日支 + 日干 + 日柱查法实现，四柱 + 大运流年覆盖；**查法来源在代码注释中注明（《三命通会》《渊海子平》惯例）并在 UI 公示流派口径**——这是"算法透明"差异化的落地；用已知命例做测试断言。
- **A1 命书 PDF（随后）**：技术选型 **jsPDF + html2canvas**——jsPDF 内置字体不支持中文、嵌入中文字体成本高，html2canvas 渲染设计稿 HTML 可保证中文与版式保真；多页 A4（封面/命盘概览/大运流年/十神/建议），含水印与免责声明。
- **A3 合婚规则完善（后续）**：纳音相合/相克、夫妻宫六合三合六冲、年月柱关系、五行缺补权重；结论改为参考式分级。
