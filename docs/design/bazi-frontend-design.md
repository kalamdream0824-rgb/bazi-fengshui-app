...
| 项目 | 内容 |
|---|---|
| 版本 | v0.20（新人解读版 · 方案 B 进行中） |
| 日期 | 2026-08-09 |
| 索引 | 见 `docs/design/README.md`；后端见 `backend-design.md` |

## 1. 当前状态

- 8 模块全可走通；测试 86/86、lint 0 问题、构建绿、CI 已配置、已上 GitHub。
...
- 🔨 **进行中：新人解读版（方案 B）**——基本排盘 + 专业细盘加"人话解读"（总览段落 + 分块要点 + 术语释义增强），设计见 §5.5，本版本评审通过后实现。
...
- v0.20（2026-08-11）：新人解读版（方案 B）设计评审中——基本排盘 + 专业细盘人话解读（`lib/explainer` 总览/分块/词库），合规边界明确；评审通过后实现。
