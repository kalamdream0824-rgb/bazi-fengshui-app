...
> 状态：已记录，暂不实施。当前优先级低于四主题输出逻辑整改。

- 每份报告允许一次免费的“明显不符”内容切换；替换内容必须在首次生成时预先计算并锁定，禁止通过重复点击随机重抽。
- 第一次反馈只替换上一年回顾，不退款、不赠送新报告，也不额外消耗会员次数。
- 第二个候选仍被反馈为明显不符时，优先返还站内生成额度；现金退款进入人工审核，不做无条件自动退款。
- 同一出生信息、同一主题、同一目标年度的重复生成不得重复获得补偿。
- 风险识别关注短时间连续反馈、重复输入、异常高的不符比例及关联账户，但不得仅凭单次“不符合”认定用户恶意。
- 高风险账户保留反馈能力，暂停自动补偿并转人工处理。
- 线上指标拆分为全量反馈率、排除异常行为后的有效不符率，以及脱敏样本盲测不符率；不得直接用全量点击量宣称准确率。
- 后续实现时必须将“内容纠错”与“经济补偿”拆成两个状态机，并补齐幂等、审计、会员次数和单次付费回归测试。
